...
def initialize_session_state():
    ...
    if "users" not in st.session_state:
        st.session_state.users = {"test": "test123"}
...
def record_speech_background(duration, queue_result):
    """Record and recognize speech in background thread - continuous recording for full duration"""
    import pyaudio
    import wave
    import io
    ...
    try:
        ...
        # Record audio continuously for the full duration
        with sr.Microphone(sample_rate=16000) as source:
            print(f"[v0] Starting continuous recording for {duration} seconds...")
            # Record continuously - don't stop on silence
            audio = recognizer.listen(source, timeout=2, phrase_time_limit=duration + 10)
            print(f"[v0] Recording complete, processing...")
            
            try:
                # Try Google Speech Recognition
                full_spoken_text = recognizer.recognize_google(audio)
                print(f"[v0] Speech recognized: {full_spoken_text}")
                queue_result.put({"status": "success", "text": full_spoken_text})
                
            except sr.UnknownValueError:
                print("[v0] Could not understand speech")
                queue_result.put({"status": "unclear", "text": "Could not understand speech clearly."})
            except sr.RequestError as e:
                print(f"[v0] API Error: {e}")
                queue_result.put({"status": "error", "text": f"API Error: {str(e)}"})
                
    except sr.WaitTimeoutError:
        print("[v0] Recording timeout - no speech detected")
        queue_result.put({"status": "timeout", "text": "No speech detected."})
    except Exception as e:
        print(f"[v0] Microphone Error: {e}")
        queue_result.put({"status": "error", "text": f"Microphone Error: {str(e)}"})


def record_and_recognize_speech(duration, passage_text, progress_placeholder):
    """Record audio and perform speech recognition - continuous recording for full duration"""
    recognizer = sr.Recognizer()
    recognizer.energy_threshold = 2000
    ...
    try:
        with sr.Microphone(sample_rate=16000) as source:
            recognizer.adjust_for_ambient_noise(source, duration=0.5)
            progress_placeholder.info(f"🎤 Recording for {duration} seconds... Speak clearly!")
            
            try:
                # Record continuously for the full duration without stopping on silence
                print(f"[v0] Starting continuous recording for {duration} seconds...")
                audio = recognizer.listen(source, timeout=2, phrase_time_limit=duration + 10)
                print(f"[v0] Recording complete, recognizing speech...")
                
                progress_placeholder.info("⏳ Processing speech... Please wait...")
                
                try:
                    # Try Google Speech Recognition
                    full_spoken_text = recognizer.recognize_google(audio)
                    print(f"[v0] Speech recognized: {full_spoken_text}")
                    accuracy = calculate_text_accuracy(passage_text, full_spoken_text)
                    progress_placeholder.success(f"Speech recognized! Accuracy: {accuracy:.1f}%")
                    
                except sr.UnknownValueError:
                    print("[v0] Could not understand speech")
                    progress_placeholder.warning("Could not understand speech clearly. Please speak louder and slower.")
                    full_spoken_text = ""
                    accuracy = 0
                except sr.RequestError as e:
                    print(f"[v0] API Error: {e}")
                    progress_placeholder.error(f"Speech recognition service error: {e}")
                    full_spoken_text = ""
                    accuracy = 0
                    
            except sr.WaitTimeoutError:
                print("[v0] Recording timeout")
                progress_placeholder.warning("No speech detected. Recording timed out.")
                full_spoken_text = ""
                accuracy = 0
                
    except Exception as e:
        print(f"[v0] Microphone error: {e}")
        progress_placeholder.error(f"Microphone error: {e}")
        full_spoken_text = ""
        accuracy = 0
    
    return full_spoken_text, accuracy
    ...
    with col2:
        ...
        if start_button:
            ...
            st.session_state.speech_data = {
                ...
                "accuracy": 0,
                "is_recording": True,
                "dyslexia_from_speech": "Recording...",
                "demo_mode": demo_mode
            }
            st.rerun()
        ...
        if st.session_state.tracking_active:
            # Create columns for camera and status
            camera_col, status_col = st.columns([2, 1])
            
            with camera_col:
                st.info("🔍 Eye tracking & speech recognition in progress... Read the passage aloud.")
                st.markdown(f"**Duration:** {reading_duration} seconds")
                ...
                if not cap.isOpened():
                    ...
                    if not st.session_state.speech_data.get("demo_mode", False):
                        # Start speech recording thread BEFORE eye tracking
                        speech_thread = threading.Thread(
                            target=record_speech_background,
                            args=(reading_duration, speech_queue),
                            daemon=True
                        )
                        speech_thread.start()
                        speech_placeholder.info("🎤 Recording speech... Speak clearly and naturally!")
                    else:
                        speech_placeholder.info("📊 Using Demo Mode for speech recognition...")
                    
                    # ===== RUN EYE TRACKING WHILE SPEECH IS RECORDING =====
                    try:
                        while st.session_state.tracking_active and (time.time() - start_time) < reading_duration:
                            ...
                            progress_text.text(f"⏱️ Elapsed: {elapsed:.1f}s / {reading_duration}s | Eye Tracking & Speech Recording ACTIVE")
                            ...
                            if results.multi_face_landmarks:
                                for face_landmarks in results.multi_face_landmarks:
                                    ...
                                    cv2.putText(frame, f"Drastic Moves: {drastic_movements_count}", 
                                              (w - 300, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                            ...
                            cv2.putText(frame, "EYE TRACKING + SPEECH REC ON", (10, h - 30),
                                      cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                    ...
                    if st.session_state.speech_data.get("demo_mode", False):
                        # Use simulated speech
                        speech_placeholder.info("⏳ Processing simulated speech...")
                        ...
                        if speech_thread:
                            speech_placeholder.info("⏳ Processing speech recording...")
                            speech_thread.join(timeout=5)
                            
                            # Get result from queue
                            try:
                                result = speech_queue.get(timeout=2)
                                if result["status"] == "success":
                                    spoken_text = result["text"]
                                    accuracy = calculate_text_accuracy(passage_text, spoken_text)
                                    speech_placeholder.success(f"Speech Recognition Complete!")
                                else:
                                    speech_placeholder.warning(f"Speech Recognition: {result['text']}")
                                    spoken_text = ""
                                    accuracy = 0
                            except queue.Empty:
                                speech_placeholder.warning("Speech recognition timed out")
                                spoken_text = ""
                                accuracy = 0
                    ...
                    # Store collected eye tracking data
                    ...
                    if len(gaze_points) > 0:
                        ...
                        st.session_state.results.update({
                            ...
                            "Speech_Analysis": {
                                ...
                                "demo_mode_used": st.session_state.speech_data.get("demo_mode", False)
                            },
                ...
                with st.spinner("Generating PDF report..."):
                    try:
                        ...
                        st.download_button(
                            ...
                            data=pdf_data,
                            file_name=f"Cognivue_Report_{st.session_state.user}_{time.strftime('%Y%m%d_%H%M%S')}.pdf",
            ...
            if st.button("📊 Export as JSON", use_container_width=True, key="json_btn"):
                try:
                    ...
                    st.download_button(
                        ...
                        data=json_str,
                        file_name=f"Cognivue_Results_{st.session_state.user}_{time.strftime('%Y%m%d')}.json",
...
def create_pdf_report():
    ...
    try:
        ...
        # Title
        elements.append(Paragraph("Cognivue Assessment Report", styles['Title']))
        ...
        **Note on Methods**:
        ...
        # Footer
        elements.append(Spacer(1, 20))
        elements.append(Paragraph("Confidential Assessment Report - Cognivue Screening Tool", 
